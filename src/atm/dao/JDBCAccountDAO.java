...
public class JDBCAccountDAO extends SimpleJdbcDaoSupport implements AccountDAO {
	...
	public Account getAccount(String cardNumber) {
		...
		try
		{
	        ...
	        String SELECT = " SELECT Account.id as id, balance, Account.accountNumber as accountNumber"
	                      + " FROM Account INNER JOIN CustomerCard on Account.accountNumber = CustomerCard.accountNumber"
	                      + " WHERE cardNumber = :cardNumber";
	        SqlParameterSource namedParameters = new MapSqlParameterSource("cardNumber", cardNumber);
